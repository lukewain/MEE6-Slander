...
async def run():
    async with asyncpg.create_pool(os.environ['PG_DSN']) as pool, aiohttp.ClientSession() as session:
        manager = SlanderManager(pool=pool, queue_channel_id=channel_id, anyone_can_click=anyone_can_click)
        async with MEE6Slander(pool=pool, slander_manager=manager, aiosession=session, dev_mode=dev_mode) as bot:
